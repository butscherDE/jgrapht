...
public class VisibilityCellTest {
    ...
    private VisibilityCell createDefaultVisibilityCell() {
        final List<Node> visibilityCellNodeIds = Arrays.asList(graph.getVertex(17), graph.getVertex(15),
                                                               graph.getVertex(18));
    ...
    private Polygon createDefaultVisibilityCellsExpectedCellShape() {
        final Coordinate[] coordinates = new Coordinate[]{new Coordinate(32, 7), new Coordinate(34, 11), new Coordinate(
                38, 7), new Coordinate(32, 7)};
    ...
    private Polygon createSurrndingPolygon() {
        final Coordinate[] coordinates = new Coordinate[]{new Coordinate(31, 6), new Coordinate(31, 12), new Coordinate(
                39, 12), new Coordinate(39, 6), new Coordinate(31, 6)};
        return gf.createPolygon(coordinates);
    }
    ...
    private Polygon createIntersectingPolygon() {
        final Coordinate[] coordinates = new Coordinate[]{new Coordinate(34, 6), new Coordinate(34, 8), new Coordinate(
                36, 8), new Coordinate(36, 6), new Coordinate(34, 6)};
        return gf.createPolygon(coordinates);
    }
    ...
    private VisibilityCell createVisibilityCellOtherThanDefaultVisibilityCell() {
        final List<Node> vcNodeId = Arrays.asList(graph.getVertex(34), graph.getVertex(15), graph.getVertex(17));
    ...
    public void subGraphBuilding() {
        ...
        final List<ReflectiveEdge> vc1edges = Arrays.asList(new ReflectiveEdge(0, vertex0, vertex1, originalGraph),
                                                            new ReflectiveEdge(1, vertex1, vertex7, originalGraph),
                                                            new ReflectiveEdge(2, vertex7, vertex0, originalGraph));

        final List<ReflectiveEdge> vc2edges = Arrays.asList(new ReflectiveEdge(3, vertex1, vertex8, originalGraph),
    ...
    public void sortedLineSegments() {
        final Coordinate[] polygonCoordinates = new Coordinate[]{new Coordinate(0, 0), new Coordinate(2,
                                                                                                      0), new Coordinate(
                2, 2), new Coordinate(1, 2), new Coordinate(0, 0)};
        ...
        final List<LineSegment> expectedSegments = Arrays.asList(new LineSegment(0, 0, 1, 2),
    ...
    public void dumpTest() {
        final VisibilityCell vc = getDumpVc();

        final String expected = getExpectedDump();
        final String dump = vc.dump();
    ...
    public void reimportTest() {
        final VisibilityCell dumpVc = getDumpVc();
        final String dump = dumpVc.dump();

        final VisibilityCell reimportedVc = VisibilityCell.create(dump, graphMocker.graph);

        assertEquals(dumpVc, reimportedVc);
    }

    public VisibilityCell getDumpVc() {
        final RoadGraph graph = graphMocker.graph;
        final List<Node> nodes = Arrays.asList(graph.getVertex(0), graph.getVertex(1), graph.getVertex(7));
        final List<ReflectiveEdge> edges = Arrays.asList(
                new ReflectiveEdge(graph.getEdge(nodes.get(0), nodes.get(1)), graph),
                new ReflectiveEdge(graph.getEdge(nodes.get(1), nodes.get(2)), graph),
                new ReflectiveEdge(graph.getEdge(nodes.get(2), nodes.get(0)), graph));
        return VisibilityCell.create(nodes, edges);
    }
